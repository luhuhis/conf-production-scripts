...
# TODO figure out how many nodes we need
...
#36 --CheckConf_path ~/code/SIMULATeQCD/build_gnu/applications/CheckConf \
...
--step_size 0.2 --no_md 5 --no_step_sf 5 --no_sw 5 \
--rat_file $rat_file \
--rand_file auto --rand_flag 1 --load_conf 0 --write_every 1 --conf_nr auto --seed random \
--always_acc 1 \
--jobname RHMC_${beta}_${Ns}${Nt}${stream} --mail_user laltenkor@bnl.gov \
--time 12:00:00 --nodes 4 --gpuspernode 4 --array "0-0%1" \
--account m3760_g --qos regular --sbatch_custom "constraint gpu" \
--custom_cmds "export MPICH_GPU_SUPPORT_ENABLED=1;" \
--no_updates 1000 \
--save_jobscript jobscript_${Ns}${Nt}_${beta_str}_${stream}.sh
